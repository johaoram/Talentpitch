...
#3. Creating database in Mysql if not exists
...
filename = "challenges - challenges.csv"

# 4.1 Reading csv challenge file and creating the table
with open(filename, 'r') as csvfile:
    ...
    for i, col in enumerate(header):
        ...
        if value.isdigit(): # If column is digit
            num_value = int(value)
    ...
    # 4.3 creating challenge details table
    sql_table = "CREATE TABLE IF NOT EXISTS challengesdetails (" + ', '.join(columns) + ")"
    cursor.execute(sql_table)

    placeholders = ', '.join(['%s'] * len(first_row))

    # 4.4 Insert data into table
    sql_insert = f"INSERT INTO  challengesdetails VALUES ({placeholders})"
