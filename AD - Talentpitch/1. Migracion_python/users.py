...
# 4.1 Reading csv users file and creating the table
with open(filename, 'r') as csvfile:
    ...
    sql_table = "CREATE TABLE IF NOT EXISTS users (" + ', '.join(columns) + ")"
    cursor.execute(sql_table)

    placeholders = ', '.join(['%s'] * len(first_row))

    # 4.4 Insert data into tabl
    sql_insert = f"INSERT INTO  users VALUES ({placeholders})"
